...
$repo = Get-DbcConfigValue -Name setup.testrepo
...
$regex = [regex]'-Tags[\s]*(.+)[\s]*\$filename'
$parsedtags = Get-Content C:\github\dbachecks\checks\*.Tests.ps1 | Where-Object { $_ -match "-Tag" } | Select-String -Pattern '-Tags[\s]*(.+)[\s]*\$filename' | ForEach-Object { $_.matches.Groups[1].Value }
$alltags += $parsedtags.Split(",").Trim() | Where-Object { $_.length -gt 2 } | Select-Object -Unique
Set-PSFConfig -Module dbachecks -Name autocomplete.pestertags -Value $alltags
