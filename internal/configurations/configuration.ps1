...
Set-PSFConfig -Module dbachecks -Name skip.agent.history -Validation bool -Value $false -Initialize -Description "Skip the Agent Job History test (useful when you don't have the job history configured - MaximumJobHistoryLogSize = -1)"
...
Set-PSFConfig -Module dbachecks -Name agent.history.maximumjobhistorylogsize -Value 1000 -Initialize -Description "Maximum job history log size (in rows). The value -1 means disabled"
Set-PSFConfig -Module dbachecks -Name agent.history.maximumjobhistoryrowsperjob -Value 100 -Initialize -Description "Maximum job history row per job. When the property is disabled the value is 0."
