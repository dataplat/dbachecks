        ...
        Context "$($PSItem.Name) - Checking the Its" {
            ...
            @($Its).ForEach{
                ...
                It "The It Title - $Title - Should end with the right ending so that the PowerBi will work correctly" {
                    ...
                    $Lower.Endswith('psitem"') -or $Lower.Endswith('clustername"') -or $Lower.EndsWith('server)"') -or $Lower.EndsWith('psitem.instancename)"') -or $Lower.EndsWith('instance"') -or $Lower.EndsWith('instance)"') -or $Lower.EndsWith('instance)"') -or $Lower.EndsWith('domain)"')  -or $Lower.EndsWith('domain"')  -or $Lower.EndsWith('psitem.computername)"') -or $Lower.EndsWith('parent.name)"')| Should -BeTrue -Because 'You need to alter the title of the It, it should end with the instance name or computername - This helps the PowerBi to parse the data'
                }
            }
        }
