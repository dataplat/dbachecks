...
Describe "SQL Engine Service Account" -Tags SqlEngineServiceAccount, ServiceAccount, $filename {
...
Describe "SQL Browser Service Account" -Tags SqlBrowserServiceAccount, ServiceAccount, $filename {
	(Get-SQLInstance).ForEach{
		Context "Testing $psitem" {
...
Describe "AdHoc Workload Optimization" -Tags AdHocWorkload, $filename{
	(Get-SQLInstance).ForEach{
		Context "Testing $psitem" {
